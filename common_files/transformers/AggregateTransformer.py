...
class AggregateTransformer(TransformerMixin):
    ...
    def transform(self, X, y=None):
        ...
        # dt_transformed.to_csv(os.path.join(self.results_dir, 'dt_transformed_agg_%s_%s.csv'%(self.model, self.dataset_name)),  index=False, sep=';')
        dt_transformed.to_parquet(os.path.join(self.results_dir, 'dt_transformed_agg_%s_%s.parquet'%(self.model, self.dataset_name)))
