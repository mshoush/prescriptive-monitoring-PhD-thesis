...
class BaseGenModel(object, metaclass=BaseGenModelMeta):
    ...
    def get_univariate_quant_metrics(self, dataset=TRAIN, transformed=False, verbose=True,
        ...
        print("\n=================metrics=================\n")
        print("t_true: ", t_true.shape)
        print("t_model: ", t_model.shape)
        ...
        nan_indices_y_true = np.where(np.isnan(y_true))[0]
        nan_count_y_true = len(nan_indices_y_true)

        nan_indices_y_model = np.where(np.isnan(y_model))[0]
        nan_count_y_model = len(nan_indices_y_model)

    

        if nan_count_y_true > 0:
            print("nan_indices_y_true: ", nan_indices_y_true)
            print("nan_count_y_true: ", nan_count_y_true)

        if nan_count_y_model > 0:
            print("nan_indices_y_model: ", nan_indices_y_model)
            print("nan_count_y_model: ", nan_count_y_model)

        try:
            y_model = np.nan_to_num(y_model)
