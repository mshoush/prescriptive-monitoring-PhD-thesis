...
DIR = './results_realcasue_3/plots/'
DIR = "./results_realcause/plots"
DPI = 300
...
def compare_marginal_hists(x1, x2, label1=None, label2=None, ax=None):
    ...
    # sns.distplot(y1_selected, ax=ax, label=label1)
    # sns.distplot(y2_selected, ax=ax, label=label2)
        
    if is_binary(x1, x2):
        sns.distplot(x1[x1 == 0], kde=False, ax=ax, label=label1)
        sns.distplot(x2[x2 == 0], kde=False, ax=ax, label=label2)
    else:
        try:
            sns.distplot(x1[x1 == 0], ax=ax, label=label1)
        except RuntimeError:
            sns.distplot(x1[x1 == 0], ax=ax, label=label1, kde_kws={'bw': 0.5})
        try:
            sns.distplot(x2[x2 == 0], ax=ax, label=label2)
        except RuntimeError:
            sns.distplot(x1[x1 == 0], ax=ax, label=label1, kde_kws={'bw': 0.5})
