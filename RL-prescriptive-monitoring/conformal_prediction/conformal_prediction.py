...
class conformal_prediction:
    ...
    def get_pred_set_adaptive(self, q_yhat):
        ...
        for i in range(y_pred.shape[0]):
            ...
            #print(self.q_yhat)
            #print(np.where(np.cumsum(np.sort(y_pred[i])[::-1]) > self.q_yhat))
            #try:
...
def main(dataset_name, pred_test, pred_train, pred_cal, dt_test_prefixes, dataset_manager, results_causal, test_file_causal):
    ...
    #print("ADAPtive...")
    #qhat_adaptive ={} 
    #qhat_adaptive = {a: cp.get_qhat_adaptive(a) for a in alpha}
    #print(qhat_adaptive)
    ...
    #print(pred_sets_adaptive)
    #raise SystemExit("ADAPTIVE!")
    ...
    print("Read ORF DATA...")
    orf_data = pd.read_csv(os.path.join(results_causal,  test_file_causal), sep=';')
    ...
    print(orf_data.columns)
    try:
        orf_data = orf_data[['NumberOfOffers', 'prefix_nr', 'case_id', 'CATE', 'Treatment', 'activity', 'timestamp', 
            "Propensity", "Proba_if_Treated", "Proba_if_Untreated", "upper_cate", "lower_cate"]] 
    except:
        orf_data = orf_data[['prefix_nr', 'case_id', 'CATE', 'Treatment', 'activity', 'timestamp',  
            "Propensity", "Proba_if_Treated", "Proba_if_Untreated", "upper_cate", "lower_cate"]] 
    ...
    #      orf_data = orf_data[['Outcome', 'prefix_nr', 'case_id', 'CATE', 'Treatment', 'activity', 'timestamp', 
    #      "Propensity", "Proba_it_Treated", "Proba_if_Untreated"]]
    # except:
...
if __name__ == "__main__":
    ...
    results_pred = argv[2]
    results_conformal = argv[3] # "results_conformal" + "/" + dataset + "/"
    results_causal = argv[4]
    ...
    for dataset_name in datasets:
        ...
        test_file_causal = "df_results_test_lower_upper_cate_%s.csv"% dataset_name
        # test prefixes pkl
        test_pfx_pkl_file = "dt_test_prefixes_%s.pkl"% dataset_name

        print(results_pred)
        pred_test = pd.read_csv(os.path.join(results_pred,  test_file), sep=';')
        pred_train = pd.read_csv(os.path.join(results_pred, train_file), sep=';')
        pred_cal = pd.read_csv(os.path.join(results_pred, cal_file), sep=';')

        with open(os.path.join(results_pred, test_pfx_pkl_file), "rb") as fh:
            dt_test_prefixes = pickle.load(fh)

        df_naive, df_classbalanced, df_adaptive, all_df = main(dataset_name, pred_test, pred_train, pred_cal, dt_test_prefixes, dataset_manager, results_causal, test_file_causal)

        if dataset_name =="bpic2017":
            print("bpic17")

        df_naive.to_csv(os.path.join(results_conformal, "df_final_naive_all_%s.csv"%dataset_name), sep=";", index=False)
        df_classbalanced.to_csv(os.path.join(results_conformal, "df_final_classBalanced_all_%s.csv"%dataset_name), sep=";", index=False)
        df_adaptive.to_csv(os.path.join(results_conformal,  "df_final_adaptive_all_%s.csv"%dataset_name), sep=";", index=False)
